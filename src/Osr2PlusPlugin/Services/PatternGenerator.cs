...
/// Position-mapped modes (Grind, ReverseGrind) take the L0 stroke position
...
public static class PatternGenerator
{
    ...
    /// or normalized L0 stroke position (0.0–1.0) for Grind/ReverseGrind.</param>
    /// <returns>Position 0.0–1.0</returns>
    public static double Calculate(AxisFillMode fillMode, double t)
    {
        // Grind/ReverseGrind use t as position input — clamp rather than wrap
        if (fillMode == AxisFillMode.Grind)
            return Math.Clamp(t, 0.0, 1.0);
        if (fillMode == AxisFillMode.ReverseGrind)
    ...
    /// <summary>Saw: linear ramp 0→1, instant drop.</summary>
    private static double CalculateSaw(double t)
        => t;

    /// <summary>Sawtooth Reverse: instant snap to 1, linear ramp down 1→0.</summary>
    private static double CalculateSawtoothReverse(double t)
        => 1.0 - t;

    /// <summary>Square: instant alternation — first half = 1, second half = 0.</summary>
